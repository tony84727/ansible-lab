...
function ensure_host_keys() {
	[[ -e "/etc/ssh/ssh_host_rsa_key" ]] || ssh-keygen -q -f /etc/ssh/ssh_host_rsa_key -b 4096
	[[ -e "/etc/ssh/ssh_host_ecdsa_key" ]] || ssh-keygen -q -f /etc/ssh/ssh_host_ecdsa_key -t ecdsa
	[[ -e "/etc/ssh/ssh_host_ed25519_key" ]] || ssh-keygen -q -f /etc/ssh/ssh_host_ed25519_key -t ed25519
}
